...
export class Sticker {
    ...
    constructor (file: string | Buffer | Readable, config: Partial<API.IConfigSticker & API.ConfigStickerVideo> = { author: "I`am Ra", pack: "RA BOT" }) {
        this.file = file;
        this.config = config;
    }
    ...
    public build = async (): Promise<string> => {
        return new Bluebird<string>(async (resolve, reject) => {
            ...
            axios({
                url: `${Sticker.DEFAULT_URL}${file.mime.startsWith("image") ? "prepareWebp" : "convertMp4BufferToWebpDataUrl"}`,
                method: "POST",
                maxBodyLength: 20000000, 
                maxContentLength: 1500000,
                headers: this.setHeaders,
                data: JSON.stringify(Object.assign(this.createMetadata(file.mime), { [`${file.mime.startsWith("image") ? "image" : "file"}`]: `data:${file.mime};base64,${file.file.toString("base64")}` }))
            }).then(respon => {
                resolve((file as API.ILoadSticker).mime.startsWith("image") ? respon.data.webpBase64 : respon.data);
            }).catch(err => {
                file = null;
                reject(err);
            }).finally(() => {
    ...
    private createMetadata=  (mime: MimeType): API.IMetadataSticker => {
        ...
        if (typeof this.config.keepScale === "undefined") this.config.keepScale = true;
        if (typeof this.config.circle === undefined) this.config.circle = false;
        if (typeof this.config.video === "undefined") this.config.video = {};
        if (typeof this.config.video.crop === "undefined") this.config.video.crop = this.config.keepScale;
        if (typeof this.config.video.fps === "undefined") this.config.video.fps = 10;
        if (typeof this.config.video.startTime === "undefined") this.config.video.startTime = "00:00:00";
        if (typeof this.config.video.endTime === "undefined") this.config.video.endTime = "00:00:7.0";
        if (typeof this.config.video.loop === "undefined") this.config.video.loop = 0;
        return (mime.startsWith("image") ? { stickerMetadata: lodash.omit(this.config, "video")} : {processOptions: this.config.video, stickerMetadata:lodash.omit(this.config, "video") }) as API.IMetadataSticker;
    }
}