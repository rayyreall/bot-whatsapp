...
export default class extends Command implements Whatsapp.MyCmd {
	...
	override async execute(client: Whatsapp.ClientType): Promise<any> {
        ...
        let cmd: any = lodash.mapValues(lodash.groupBy(events, "group"), (c) => lodash.map(c, (v2) => {
            if (typeof v2.help === "string") return  v2.costumePrefix?.isPrefix  ? { prefix: `${v2.costumePrefix?.prefix ? v2.costumePrefix.prefix : `${prefix?.prefix || "."}`}${v2.help}`} : { noprefix: v2.help}
            return v2.help.map((value: string) => v2.costumePrefix?.isPrefix ? { prefix: `${v2.costumePrefix?.prefix ? v2.costumePrefix.prefix : `${prefix?.prefix || "."}`}${value}`} : {noprefix: value});
        }).flat())
        cmd =  lodash.mapValues(cmd, (c) => c.reduce((acc: any, v: any) => {
